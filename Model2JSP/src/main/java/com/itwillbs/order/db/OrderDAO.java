...
public class OrderDAO {
	...
	// 공통의 멤버변수(전역변수,인스턴스변수)
		...
		public void addOrder(OrderDTO ordto, List basketList, List goodsList){
			...
			// 싱글톤 패턴
			// 추상메소드라서 new로 객체를 만들지 못함.
			// getInstance는 static 메소드 - 직접 객체를 만들지는 못해도 만들어진 객체를 가져다 쓸 수는 있다.
			...
			try {
				...
				System.out.println("DAO 일련번호 : " + o_num);
				...
				for(int i=0; i<basketList.size();i++) {
					...
					pstmt.setInt(3, bkdto.getB_g_num()); // 구매상품 번호
					pstmt.setString(4, gdto.getName()); // 구매상품
		...
		public List getOrderDetailList(String trade_num){
				...
				try {
					...
					System.out.println("DAO : 상세 주문 리스트 저장 완료");
